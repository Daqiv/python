...
#日志工具
LOG_PATH = 'log/'+wechat_user+'/'
mkdir(LOG_PATH)
logger = Logger(LOG_PATH+'all.log',level='debug').logger


logger.info('Hello,'+wechat_user)
...
newInstance.auto_login(hotReload=True, statusStorageDir="log/"+wechat_user)
...
def xml_filer(msg_xml):
	result = {}
	log = 'Now will deal msg_xml..'+hashlib.md5(msg_xml.encode(encoding='UTF-8')).hexdigest()
	logger.info(log)
			...
			result['data'].update(
				{
					i:{
						...
						"pub_time":item.find('pub_time').text,
...
#捕获器(注册SHARING类型监听)
@newInstance.msg_register([itchat.content.SHARING],isMpChat=True)
def handle_receive_msg(msg):
	...
	if msg['MsgType'] == 49:
		...
		msg_from = newInstance.search_mps(userName=msg['FromUserName'])['NickName']  #在公众号列表中查询发送信息的昵称
...
#itchat.run(True)
#while True:
#	login_stat = newInstance.load_login_status(fileDir="log/"+wechat_user)
#	print( login_stat )
#	time.sleep(1)
