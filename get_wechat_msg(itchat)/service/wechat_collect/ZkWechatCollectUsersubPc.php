...
class ZkWechatCollectUsersubPc extends ZkWechatCollect
{
    ...
    public function main(){
        ...
        if(!empty($this->biz)){
                ...
                if ($oArticle['app_msg_ext_info']['is_multi'] == 1) {
                    foreach ($oArticle['app_msg_ext_info']['multi_app_msg_item_list'] as $oInnerArticle) {
                        ...
                        foreach ($arrCollectJob as $oJob) {
                            ...
                            if(FILE_PUT_CONTENTS_DEBUG==1) {
                                file_put_contents(dirname(__FILE__).'/'.date('Y-m-d') . "_check.log", $oJob['app_id'] . '-' . $oJob['article_be_enabled'] . '-' . $innerArticleData['title'] . '-' . $oJob['name'] . '-' . $innerArticleData['unable'] . '=' . $innerArticleData['stat'] . "\n", FILE_APPEND);
                                file_put_contents(dirname(__FILE__).'/'.date('Y-m-d') . "_list_log.log", var_export($innerArticleData, true), FILE_APPEND);
                            }
                            //入库
                            $mongo = db_mongoDB_conn(ZK_MONGO_TB_ARTICLE_WECHAT);
            ...
            if($articleCount>0){
                $logger->addDebug('WechatCollectPC_YES', array("come_from"=>"weixin_pc","biz"=>$this->biz,"num"=>$articleCount,"time"=>date('Y-m-d H:i:s'),'web'=>$_REQUEST['web'],'ext'=>$oJob));
            }else{
                $logger->addDebug('WechatCollectPC_NO', array("come_from"=>"weixin_pc","biz"=>$this->biz,"num"=>$articleCount,"time"=>date('Y-m-d H:i:s'),'web'=>$_REQUEST['web'],'ext'=>$oJob));
            }
        ...
        if(defined('USE_FAST_MODEL') && USE_FAST_MODEL){
            ...
            if(!empty($_REQUEST['web']) && intval($_REQUEST['web'])==1){
                //$_REQUEST['web'])==1 扩展带参数特殊处理,主要是给 按键精灵 模式进行使用
                list($biz,$redirect_time) = self::getLongestNoSyncBizExt($this->urgent);
                if($redirect_time==0){
                    $redirect_time = self::getRedirectTime() * 2; //由于考虑到同一个号手机端也在轮询，把PC端时间放大2倍
                }
                $url = "https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=" . $biz . "&scene=124#wechat_redirect";
                echo $url;

            }else if(!empty($_REQUEST['web']) && intval($_REQUEST['web'])==2){
                //$_REQUEST['web'])==2 扩展带参数特殊处理,主要给 微信检测新的订阅公众号信息
                $biz = self::getNewlyWechat();
                if($biz){
                    $url = "https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=" . $biz . "&scene=124#wechat_redirect";
                    //$test = file_get_contents($url);
                    //echo "<script>setTimeout('window.location.href=\"$url\"',5000);</script>";
                    echo $url;
                }else{
...
//
     ...
     * @return bool
     */
    public function getNewlyWechat(){
        $db = db_mysql_conn(ZK_MYSQL_TB_USER_WX_FEED_BURN);
        $db->trans_start();
        $db->where('stat', 1);
        $db->where('newly_check',1);
        $db->order_by('last_sync_time', 'ASC');
        $db->limit(1);
        $query = $db->get(ZK_MYSQL_TB_USER_WX_FEED_BURN);
        $collectJob = $query->row_array();
        $updateData = array('last_sync_time' => date("Y-m-d H:i:s"),'newly_check'=>2);//暂时改为2,说明正在处理中,处理完成再改为0
        $db->where('id', $collectJob['id']);
        $db->update(ZK_MYSQL_TB_USER_WX_FEED_BURN, $updateData);
        $db->trans_complete();
        if($collectJob['biz']){
